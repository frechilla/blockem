...
void BlockemChallengeTest::LoadAndCheckValidChallenge()
{
    ...
    assert (opponentTakenSet.size() == 8);
    assert (opponentTakenSet.isPresent(Coordinate(1,  1)));
    assert (opponentTakenSet.isPresent(Coordinate(1, 12)));
    assert (opponentTakenSet.isPresent(Coordinate(12, 1)));
    assert (opponentTakenSet.isPresent(Coordinate(12, 12)));
    assert (opponentTakenSet.isPresent(Coordinate(6,  6)));
    assert (opponentTakenSet.isPresent(Coordinate(6,  7)));
    assert (opponentTakenSet.isPresent(Coordinate(7,  6)));
    assert (opponentTakenSet.isPresent(Coordinate(7,  7)));
    ...
    assert(takenSquaresUnionSet.empty() == true);
    ...
    for (int32_t i = e_minimumPieceIndex; i < e_numberOfPieces; i++)
    {
        ...
        {
            assert (theChallenge.IsOpponentPieceAvailable(i) == false);
        }
    }
    ...
    for (int32_t i = e_minimumPieceIndex; i < e_numberOfPieces; i++)
    {
        if ( (i != e_1Piece_BabyPiece) &&
             ...
             (i != e_5Piece_Cross)     )
        {
            assert (theChallenge.IsOpponentPieceAvailable(i) == true);
        }
    }
}
